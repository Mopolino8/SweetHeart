                               ...
                               const std::string & unknown_name)
{
 ...
 const unsigned int flag = parameters.get<unsigned int> ("exact solution flag");
    ...
    if (flag == 8){
        return (x*x + y*y - 1)*exp(x*y)*sin(z);        
    }
...
int main (int argc, char** argv)
{
  ...
  if(exact_sol_flag == 8)
  {
      if(dim < 3 || domain_type != "cube")
      {
          std::cout << "\n";    
          std::cout << "need dim = 3 and domain_type = cube for this example!\n";
          std::cout << "\n";    
          return 0;
      }
  }
  ...
  for (int nn = 0; nn < num_refinement_levels; nn++)
  {
      ...
      // build mesh of unit cube
      if(domain_type.compare("cube") == 0)
      {
          // build triangular mesh in the square [0,1]^2
          MeshTools::Generation::build_cube(mesh, nx, nx, nx, 0.0, 1.0, 0.0, 1.0, 0.0, 1.0, TET4);
      }
